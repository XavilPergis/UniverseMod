	...
	LENTICULAR("lenticular") {
		...
		public DensityField3 createDensityField(Random random) {
			...
			final var centralBulge = DensityField3.mul(
					DensityField3.uniform(50), DensityField3.sphereCloud(radius * ellipticalFactor));
			final var thinDiscField = DensityField3.min(
					DensityField3.cylinderMask(radius, radius * discHeightFactor),
					DensityField3.uniform(15));

			// var combined = DensityField3.mul(DensityField3.max(DensityField3.uniform(0.03), noise), DensityField3.add(centralBulge, thinDiscField));
