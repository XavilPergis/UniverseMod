package net.xavil.ultraviolet.client.gl.shader;
...
public final class ShaderLoader {
	...
	public static final ResourceLocation STANDARD_LOCATION = Mod.namespaced("standard.glsl");
	...
	private static void appendInclude(Parser parser, StringBuilder output, ResourceLocation location, int line)
		...
		Mod.LOGGER.debug("applying include directive for shader '{}': #include [{}]",
	...
	private static final class UnspecializedSource {
		...
		public ImmutableList<ShaderStage.Stage> getStages() {
			...
			if (this.hasTessControlStage) Mod.LOGGER.warn("tesselation control shaders are not supported in OpenGL 3.2");
			if (this.hasTessEvalStage) Mod.LOGGER.warn("tesselation evaluation shaders are not supported in OpenGL 3.2");
			...
			if (this.hasComputeStage) Mod.LOGGER.warn("compute shaders are not supported in OpenGL 3.2");
			// @formatter:on
			return stages;
		}
	}
	...
	public static final ShaderProgram load(ResourceProvider provider, ResourceLocation location, VertexFormat format,
		...
		try {
			...
			for (final var stageType : source.getStages().iterable()) {
				...
				// Mod.LOGGER.warn("compiling stage {} '{}' ('IS_{}_STAGE')", stageType.id,
