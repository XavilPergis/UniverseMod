#ifndef ULTRAVIOLET_NOISE_H_
#define ULTRAVIOLET_NOISE_H_
...
vec2 noiseSimplex2(vec2 v, float seed) {
	return vec2(
		...
		noiseSimplex(vec3(v, seed + 131.4))
	);
}

vec3 noiseSimplex3(vec3 v, float seed) {
	return vec3(
		noiseSimplex(vec4(v, seed + 894.4)),
		noiseSimplex(vec4(v, seed + 567.2)),
		noiseSimplex(vec4(v, seed + 179.7))
	);
}
