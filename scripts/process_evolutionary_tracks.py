...
# list(T): [count: u16] [data: *T]

# tracks: list(isochrone)
# isochrone: [log_age: f32] [entries: list(iso_entry)]
# iso_entry: [initial_mass: f32] [final_mass: f32] [log_luminosity: f32] [log_temperature: f32] [log_radius: f32]
...
with OUTPUT_PATH.open('wb') as out:
	...
	for iso in isos:
		...
		for entry in iso.entries:
			out.write(struct.pack('>ffffb', entry.current_mass, entry.log_L, entry.log_Teff, entry.log_R, int(entry.phase)))
