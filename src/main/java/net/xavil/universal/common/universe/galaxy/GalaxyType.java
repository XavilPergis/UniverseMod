...
public enum GalaxyType {
	...
	SPIRAL("spiral") {
		...
		public DensityFields createDensityFields(double galaxyAge, Random random) {
			final var radius = Units.fromLy(random.nextDouble(2000, 6000));
			...
			// var noise1 = DoubleField3.simplexNoise(0.0).scale(1.0).mul(0.5).add(0.5);
			// var noise2 = DoubleField3.simplexNoise(1.0).scale(0.02).mul(0.5).add(0.5);
			// var noise1 = DoubleField3.max(DoubleField3.uniform(0.0),
			// DoubleField3.simplexNoise(ns + 0.0).scale(1.0));
			var noise2 = DoubleField3.simplexNoise(ns + 1.0).scale(1.0 / Units.fromLy(50.0));
			noise2 = noise2.lerp(0.25, 1.0);
			noise2 = DoubleField3.max(DoubleField3.uniform(0.0), noise2);
			...
			var galacticCoreDensity = DoubleField3.sphereCloud(galacticCoreSizeFactor * radius)
					.scale(ellipticalFactor, galacticCoreSquishFactor, 1).curvePoly(4).mul(75);
			...
			galacticHalo = galacticHalo.scale(ellipticalFactor, galacticCoreSquishFactor, 1).mul(0.001);
			...
			var uniformDisc = DoubleField3.verticalDisc(radius, radius * discHeightFactor, 2)
					.scale(ellipticalFactor, 1, 1).curvePoly(2.0).mul(18.5);
			...
			var spokesDisc = DoubleField3.verticalDisc(radius, 1.5 * radius * discHeightFactor, 1)
					.scale(ellipticalFactor, 1, 1).curvePoly(2.0);
			var spokes = DoubleField3.spokes(spokeCount, 3.0 * spokeCurve).mul(35.0)
					.spiralAboutY(spiralFactor * 1e-8 / radius)
					.mul(spokesDisc);
			// var spokesAge = DoubleField3.random().curvePoly(1e5).lerp(1, galaxyAge);

			var densityCombined = galacticCoreDensity.add(galacticHalo).add(uniformDisc).add(spokes).mul(noise2);
