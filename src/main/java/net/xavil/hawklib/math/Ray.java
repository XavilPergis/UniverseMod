...
public record Ray(Vec3 origin, Vec3 dir) {
	...
	public boolean intersectsSphere(Vec3 center, double radius) {
		final var p = center.sub(this.origin);
		final var a = p.projectOnto(this.dir);
		if (p.dot(this.dir) < 0) return false;
		return p.lengthSquared() - a.lengthSquared() <= radius * radius;
